...
def plot_freq_adv_log(f,nc,c,lg):
  max = 0
  label = np.zeros(f.shape[0])
  label[0] = 0
  for i in range(1,nc+1):
    label[i] = c/100 * i
  for i in range(nc+1,2*nc):
    label[i] = c/10 * (i-nc+1)
  label[2*nc] = 10*c
  for k in range(f.shape[0]):
    if (f[k] > max):
      max = f[k]
  for k in range(f.shape[0]):
    lg.debug('{0:3f} {1} {2}'.format(label[k], '+' * int(f[k]/max*50), f[k]))

def plot_freq_adv(f,nc,c):
  max = 0
  label = np.zeros(f.shape[0])
  label[0] = 0
  for i in range(1,nc+1):
  ...
  for k in range(f.shape[0]):
    if (f[k] > max):
      max = f[k]
  for k in range(f.shape[0]):
    print('{0:3f} {1} {2}'.format(label[k], '+' * int(f[k]/max*50), f[k]))
...
def plot_h6_mix_neg(orig,dec,start,n):
  ...
  plt.show()
#
def plot_h6_mix_emb(orig,dec,emb,start,n,file,neg):
  plt.figure(figsize=(20, 8))
  for i in range(n):
    #
    # display original feature 0,2,4
    ...
    for j in range(emb.shape[1]):
      for k in range(emb.shape[2]):
        if (neg == 0):
          val = emb[start+i,j,k,0]
        else:
          val = neg - emb[start+i,j,k,0]
        embnorm[j,k,0] = val
        if (embnorm[j,k,0] < 0):
          embnorm[j,k,0] = 0
        if (embnorm[j,k,0] > 1):
        ...
        if (emb.shape[3] > 1):
          if (neg == 0):
            val = emb[start+i,j,k,1]
          else:
            val = neg - emb[start+i,j,k,1] 
          embnorm[j,k,1] = val
          if (embnorm[j,k,1] < 0):
            embnorm[j,k,1] = 0
          if (embnorm[j,k,1] > 1):
        ...
        if (emb.shape[3] > 2):
          if (neg == 0):
            val = emb[start+i,j,k,2]
          else:
            val = neg - emb[start+i,j,k,2]
          embnorm[j,k,2] = val
          if (embnorm[j,k,2] < 0):
            embnorm[j,k,2] = 0
          if (embnorm[j,k,2] > 1):
